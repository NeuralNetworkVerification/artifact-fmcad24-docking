from training_exp_comb_mask import train_model, retrain_model, LyapunovNetworkV, TwoDimDocking
from queries_comb_mask import safe_descent_cond_check
import torch
...
prev_model_file = "/barrett/scratch/udayanm/for_cluster/exp_tr_mask_pos4_0/dummy/slurm-1/models/cert_7.pt"
prev_pos_bound = 5
...
train_model(safe_pos, pos_limit, 0.5, cur_train_file, cur_val_file, cur_model_file, cur_controller_file, threshold, initial_controller_file, prev_model_file, prev_pos_bound)
...
ret, ret_ranges, failed = safe_descent_cond_check(cur_comb_file, cur_model_onnx_file, cur_models_file, prev_pos = prev_pos_bound, safe_pos = safe_pos, limit_pos = pos_limit, docking_pos = 0.35, vel_limit = 0.5)
...
while (len(ret) > 0):
    ...
    st_train_time = datetime.now()
    retrain_model(index - 1, torch.Tensor(ret), torch.Tensor(ret_ranges), safe_pos, pos_limit, 0.5, cur_train_file, next_train_file, next_val_file, cur_model_file, cur_controller_file, next_model_file, next_controller_file, threshold, prev_model_file, prev_pos_bound)
    end_train_time = datetime.now()
    diff = end_train_time - st_train_time
    f = open(out_timing_counterexample_file, "a")
    print("Total training time for model index", str(index), ":", str(diff.seconds))
    ...
    st_ver_time = datetime.now()
    ret, ret_ranges, failed = safe_descent_cond_check(next_comb_file, next_model_onnx_file, next_models_file, prev_pos = prev_pos_bound, safe_pos = safe_pos, limit_pos = pos_limit, docking_pos = 0.35, vel_limit = 0.5)
    end_ver_time = datetime.now()
    diff_ver_time = end_ver_time - st_ver_time
    f = open(out_timing_counterexample_file, "a")
    print("Total verification time for verification index", str(index), ":", str(diff_ver_time.seconds) + "\n")
