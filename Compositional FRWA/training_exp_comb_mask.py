...
class TwoDimDocking():
    ...
    def goal_mask(self, x):
        ...
        goal_mask.logical_and_(abs(x[:,2]) < self.vel_limit - 0.001)
        goal_mask.logical_and_(abs(x[:,3]) < self.vel_limit - 0.001)
        #goal_mask.logical_and_(self.goal_reached_mask(x))
        #goal_mask.logical_and(self.safe_mask(x))
        return goal_mask
    def goal_reached_mask(self, x):
        goal_mask = abs(x[:, 0]) < self.prev_pos_bound - 0.01
        goal_mask.logical_and_(abs(x[:,1]) < self.prev_pos_bound - 0.01)
    ...
    def nongoal_mask(self, x):
        ...
        nongoal_mask.logical_and_(~self.goal_reached_mask(x))
        #goal_mask.logical_and(self.safe_mask(x))
        return nongoal_mask
...
class TrainerRetrain(pl.LightningModule):
    def __init__(self, model, V, controller, datamodule, out_model, out_controller, threshold, primal_learning_rate = 1e-4, goalfactor = 1, decreasefactor = 1e1, nongoalfactor = 1, goaleps = 1e-4, nongoaleps=1e-4, descenteps = 1e-4, safe_level = 1, safe_factor = 1, unsafe_factor = 1e2, eps = 1e-5):
        super().__init__()
        self.epoch = 0
        self.automatic_optimization = False
    ...
    def goal_loss(self, x, x_c, goal_mask, nongoal_mask, safe_mask, unsafe_mask, goal_mask_c, nongoal_mask_c, safe_mask_c, unsafe_mask_c):
        ...
        if len(V_goal_reached_c) == 0:
            ...
            prev_term_c = self.safe_factor * prev_violation_c.mean()
            prev_acc_c = 0 
