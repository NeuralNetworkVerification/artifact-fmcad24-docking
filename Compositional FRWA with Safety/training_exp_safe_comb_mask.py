...
class TwoDimDocking():
    def __init__(self, st_pos, unsafe_pos, vel_limit, prev_model_file, prev_pos_bound):
        n = 0.001027
        self.v0 = 0.2
        self.v1 = 2*n
        ...
        self.prev_model = torch.load(prev_model_file)
        self.prev_pos_bound = prev_pos_bound
    ...
    def goal_mask(self, x):
        ...
        goal_mask.logical_or_(self.goal_reached_mask(x))

        safe_mask_add = abs(x[:,0]) < self.prev_pos_bound - 1
        safe_mask_add.logical_and_(abs(x[:,1]) < self.prev_pos_bound - 1)
        safe_mask_add.logical_and_(abs(x[:,2]) <= self.st_vel_limit)
        safe_mask_add.logical_and_(abs(x[:,3]) <= self.st_vel_limit)
        goal_mask.logical_or_(safe_mask_add)
        #goal_mask.logical_and(self.safe_mask(x))
        return goal_mask
    def goal_reached_mask(self, x):
        goal_mask = abs(x[:, 0]) < self.prev_pos_bound
        goal_mask.logical_and_(abs(x[:,1]) < self.prev_pos_bound)
        goal_mask.logical_and_(torch.sqrt(torch.pow(x[:,2],2) + torch.pow(x[:,3],2)) < self.gen_vel_limit(x[:,0], x[:,1]) - 0.0001)
        goal_mask.logical_and_(self.prev_model(x)[:,0] < 1)
        #goal_mask.logical_and_(~self.goal_mask(x))
        return goal_mask
    def nongoal_mask(self, x):
        #nongoal_mask.logical_and(self.safe_mask(x))
        #nongoal_mask.logical_and_(~self.unsafe_mask(x))
        nongoal_mask = ~self.goal_mask(x)
        #goal_mask.logical_and(self.safe_mask(x))
        return nongoal_mask
...
class SampleData(pl.LightningDataModule):
    def __init__(self, train_file_name, val_file_name, two_dim_docking, prev_model_onnx, num_points = 10000000, dim = 4, val_split = 0.1, batch_size = 10000, max_tries = 5000):
        super().__init__()
        self.num_points = num_points
        self.two_dim_docking = two_dim_docking
        self.prev_model_onnx = prev_model_onnx
        self.dim = dim
    ...
    def prepare_data(self):
        ...
        x = torch.Tensor(self.num_points*5, 2).uniform_(0.0, 1.01)

        for i in range(2):
            x[:, i] = x[:, i] * (2 * self.two_dim_docking.unsafe_pos) - self.two_dim_docking.unsafe_pos

        th = torch.Tensor(self.num_points*5).uniform_(0.0, 2*np.pi)
        v_l2 = torch.Tensor(self.num_points*5).uniform_(0.0, 1.01)
        ...
        print("num_typical 1 " + str(len(x)))

        random_indices = torch.randperm(len(x))

        x = x[random_indices]

        x_train = x[:3 * self.num_points//5]
        ...
        for i in range(2):
            x[:, i] = x[:, i] * (2 * self.two_dim_docking.st_pos) - self.two_dim_docking.st_pos
        ...
        #print(self.two_dim_docking.prev_model(x)[:,0])
        
        if (len(x) < self.num_points//5):
            print(x)
            print(self.two_dim_docking.st_pos)
            vals, val_ranges = queries_safe_comb_mask.run_find_nongoal_safe(self.prev_model_onnx, self.two_dim_docking.prev_pos_bound, self.two_dim_docking.st_pos)
            if (len(vals) > 0):
                x = torch.cat((x,torch.Tensor(vals)))

                for i in range(len(vals)):
                    new_points = torch.Tensor(self.num_points * 5//(4*4), 4).uniform_(0.0, 1.01)
                    for j in range(2):
                        new_points[:, j] = vals[i][j] + (0.2) * new_points[:, j] - 0.1
                    for j in range(2):
                        new_points[:, j+2] = 0

                    x = torch.cat((x,new_points))

                for i in range(len(val_ranges)):
                    new_points = torch.Tensor(self.num_points * 5//(4*4), 4).uniform_(0.0, 1.01)
                    for j in range(2):
                        new_points[:, j] = val_ranges[i][j][0] + (val_ranges[i][j][1] - val_ranges[i][j][0]) * new_points[:, j]
                    for j in range(2):
                        new_points[:, j+2] = 0
        ...
        #now sample for previous region
        x = torch.Tensor(self.num_points*5, 2).uniform_(0.0, 1.01)
        ...
        print("num prev 1 " + str(len(x)))

        if (len(x) < self.num_points//5):
            print(x)
            vals, val_ranges = queries_safe_comb_mask.run_find_nongoal_prev(self.prev_model_onnx, self.two_dim_docking.prev_pos_bound, self.two_dim_docking.vel_limit)

            if (len(vals) > 0):
                x = torch.cat((x,torch.Tensor(vals)))

                for i in range(len(vals)):
                    new_points = torch.Tensor(self.num_points * 5//(4*4*4*4), 4).uniform_(0.0, 1.01)
                    for j in range(2):
                        new_points[:, j] = vals[i][j] + (0.2) * new_points[:, j] - 0.1
                    for j in range(2):
                        new_points[:, j+2] = vals[i][j+2] + (0.1) * new_points[:, j+2] - 0.05
                ...
                not_unsafe_mask_x = ~self.two_dim_docking.unsafe_mask_init(x)
                x = x[not_unsafe_mask_x]
                print("num prev 2" + str(len(x)))

        random_indices = torch.randperm(len(x))

        x = x[random_indices]

        x_train_3 = x[:self.num_points//5]

        #finishing part 3 sampling

        x_train = torch.cat((x_train, x_train_3))

        #do remaining sampling
        len_train_so_far = len(x_train)
        len_needed = self.num_points - len_train_so_far
        ...
        x_train_4 = x[:len_needed]

        x_train = torch.cat((x_train, x_train_4))
        ...
        x = torch.Tensor(self.num_points*2//50, 2).uniform_(0.0, 1.01)
        ...
        v_l2 = torch.Tensor(self.num_points*2//50).uniform_(0.0, 1.01)
        safe_v = self.gen_vel_limit(x[:,0], x[:,1])
        v_l2 = v_l2 * (safe_v - 0.0) + 0.0
        vx = v_l2 * torch.cos(th)
        x = torch.cat((x, torch.unsqueeze(vx, 1)), 1)
        ...
        z = torch.Tensor(self.num_points*2//50, 2).uniform_(0.0, 1.01)
...
class Trainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        #print(goal_term, descent_term, nongoal_term, total_loss)
...
def train_model(st_pos, unsafe_pos, vel_limit, out_train_file, out_val_file, out_model_file, out_controller_file, threshold, initial_controller_file, prev_model_file, prev_model_onnx, prev_pos_bound, lr=5e-3):
    model = TwoDimDocking(st_pos, unsafe_pos, vel_limit, prev_model_file, prev_pos_bound)
    V = torch.load(prev_model_file)
    #V = LyapunovNetworkV(model)
    datamodule = SampleData(out_train_file, out_val_file, model, prev_model_onnx)
    controller = Controller(file_name = initial_controller_file, isInitial = False)
...
def retrain_model(epoch, counterexamples, counterexample_ranges, st_pos, unsafe_pos, vel_limit, in_train_file, out_train_file, out_val_file, in_model_file, in_controller_file, out_model_file, out_controller_file, threshold, prev_model_file, prev_pos_bound, lr=1e-4):
