...
class TwoDimDocking():
    ...
    def goal_mask(self, x):
        goal_mask = abs(x[:, 0]) < 0.35
        goal_mask.logical_and_(abs(x[:,1]) < 0.35)
        goal_mask.logical_and_(abs(x[:,2]) < self.vel_limit)
        goal_mask.logical_and_(abs(x[:,3]) < self.vel_limit)
        #goal_mask.logical_and_(self.goal_reached_mask(x))
    ...
    def nongoal_mask(self, x):
        ...
        nongoal_mask.logical_or_(~self.goal_reached_mask(x))
        #goal_mask.logical_and(self.safe_mask(x))
        return nongoal_mask
...
class Trainer(pl.LightningModule):
    ...
    def goal_loss(self, x, goal_mask, nongoal_mask, safe_mask, unsafe_mask):
        ...
        goal_reached_mask = self.model.goal_reached_mask(x)
        V_goal_reached = V[goal_reached_mask]
        if len(V_goal_reached) == 0:
            prev_violation,prev_term,prev_acc = 0,0,0
        else:
            prev_violation = F.relu(self.eps + V_goal_reached - self.safe_level)
            prev_term = self.safe_factor * prev_violation.mean()
            prev_acc = 0 
        safe_term += prev_term
        safe_acc += prev_acc
        '''
...
class SampleDataRetrain(pl.LightningDataModule):
    def __init__(self, two_dim_docking, datapoints, counterexample_ranges, in_train_file, train_file, val_file, num_dpoints, num_points = 10000000, dim = 4, val_split = 0.1, batch_size = 10000, max_tries = 5000):
        super().__init__()
        self.num_points = num_points
        self.two_dim_docking = two_dim_docking
        self.dim = dim
    ...
    def prepare_data(self):
        ...
        x_counterexamples = torch.Tensor(10*num_added, self.dim).uniform_(
            0.0, 1.0
        )
...
class TrainerRetrain(pl.LightningModule):
    def __init__(self, model, V, controller, datamodule, out_model, out_controller, threshold, primal_learning_rate = 1e-5, goalfactor = 1, decreasefactor = 1e1, nongoalfactor = 1, goaleps = 1e-4, nongoaleps=1e-4, descenteps = 1e-4, safe_level = 1, safe_factor = 1, unsafe_factor = 1e2, eps = 1e-5):
        super().__init__()
        self.epoch = 0
        self.self.automatic_optimization = False
    ...
    def goal_loss(self, x, x_c, goal_mask, nongoal_mask, safe_mask, unsafe_mask, goal_mask_c, nongoal_mask_c, safe_mask_c, unsafe_mask_c):
        ...
        goal_reached_mask_c = self.model.goal_reached_mask(x_c)
        V_goal_reached_c = V[goal_reached_mask_c]
        if len(V_goal_reached_c) == 0:
            prev_violation_c,prev_term_c,prev_acc_c = 0,0,0
        else:
            prev_violation_c = F.relu(self.eps + V_goal_reached_c - self.safe_level)
            prev_term_c = torch.sum(self.safe_factor * prev_violation_c)
            prev_acc_c = 0 
        safe_term += prev_term_c
        safe_acc += prev_acc_c
        ...
        if len(V_unsafe_c) == 0:
            ...
            unsafe_term_c = torch.sum(self.unsafe_factor * unsafe_violation.mean())
            unsafe_acc_c = 0
...
def train_model(st_pos, unsafe_pos, vel_limit, out_train_file, out_val_file, out_model_file, out_controller_file, threshold, initial_controller_file, prev_model_file, prev_pos_bound, lr=1e-3):
...
def retrain_model(counterexamples, counterexample_ranges, st_pos, unsafe_pos, vel_limit, in_train_file, out_train_file, out_val_file, in_model_file, in_controller_file, out_model_file, out_controller_file, threshold, prev_model_file, prev_pos_bound, lr=1e-5):
    #counterexamples = read_in_points(in_data_file, in_folder)
    model = TwoDimDocking(st_pos, unsafe_pos, vel_limit, prev_model_file, prev_pos_bound)
    V = torch.load(in_model_file)
    datamodule = SampleDataRetrain(model, counterexamples, counterexample_ranges, in_train_file, out_train_file, out_val_file, 10000)
